...
// 1. HIGH SCORE SYSTEM: Reads from a local text file
...
// 2. VECTOR FONT RENDERER: Draws characters using raw OpenGL line strips
...
void update_game() {
    ...
    // Next Level Check logic
    if (player.y < 40) {
        if (level >= MAX_LEVEL) {
            game_won = true;
            if (!win_sound_played) {
                system("kill $(cat .music.pid) 2>/dev/null; rm .music.pid 2>/dev/null");
